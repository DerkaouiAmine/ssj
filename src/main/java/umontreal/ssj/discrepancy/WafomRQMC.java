...
public class WafomRQMC {
    ...
    public WafomRQMC (DigitalNetBase2 dn, double c, int s,int w,int k) {
        this.dn=dn;
        this.c=c;
        this.s=s;
        this.w=w;
    ...
    private static double calcWafomSub(int[] point) {
        ...
        for (int i = 0; i < s; i++) {
            ...
            for (int j = 0; j < N; j++) {
                ...
                double p = 1.0 + m1p(bij) * Math.pow(2.0, -2*j);
                prod *= p;
            }
            startIndex++;
...
//j'ai le produit partiel et je stocke a la fin la somme
    ...
    //Wafom partielle ie il calcule pour 2^{k1} nombre de points mais sa sert a rien puisqu'on crer le new Wafom avec le bon k donc ici le but c'est de donner un tableau au lieu de dn 
