...
public class GenzfunctionTestWithBestPointSets implements MonteCarloModelDouble {
	...
	public static void main(String[] args) throws IOException {
		...
		/**
		 * Here, adjustments are needed based on the dimension. Referring to Genz Storage:
		 * - For s=3, choose method < 7
		 * - For s=6, choose method < 5
		 * - For s=16, choose method < 3
		 */
			...
			while(BestOrWorstWaf<2) {
				...
				for (int k1=1;k1<=Generamatrix[0].length;k1++) {
					...
					for (int i=0;i<nbreSet;i++) {
						...
						RQMCExperiment.simulReplicatesRQMC(new GenzfunctionTestWithBestPointSets (dim,w, k,  nbreSet, aTotal[i], uTotal[i]),  Sob, rand, replicates,
								stats1) ;
