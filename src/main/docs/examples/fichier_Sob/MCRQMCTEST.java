...
public class MCRQMCTEST implements MonteCarloModelDouble {
	static int dim;
	double sum;
	 int nbreSet;
	 int w;
	...
	// Constructor.
	public MCRQMCTEST (int dim,int w,int k, int nbreSet,double[] a, double[] u) {
	...
	public static void main(String[] args) throws IOException {
		...
		Tally stats=new Tally("Simul MC");
		 RandomStream random = new MRG32k3a();
	      ...
	      int nbreSimulation=(1<<k);

	    //  int nbreSimulation=(int) Math.pow(10, 7);
	      double [] moyenneMc=new double [nbreSet];
	      double [] varianceMc=new double [nbreSet];
	      
	      System.out.println("******************Methode MC****************");

	      for (int i=0;i<nbreSet;i++) {
	    	  System.out.println("le nombre de Set "+(i+1));
	    	  stats.init();
	 		 RandomStream random1 = new MRG32k3a();

	      MonteCarloExperiment.simulateRuns (new MCRQMCTEST (dim,w, k,  nbreSet, aTotal[i], uTotal[i]), nbreSimulation, random1,
	  	        stats);
	      
	      moyenneMc[i]=stats.average();
	      varianceMc[i]=stats.variance();
	      
	      System.out.println("Moyenne = "+stats.average()+" VAriance = "+stats.variance());
	      ...
	      int[][] Generamatrix =  {
			...
			/*RQMCExperiment.simulReplicatesRQMC(new MCRQMCTEST (dim,w, k,  nbreSet, aTotal[0], uTotal[0]),  p, rand, replicates,
					stats1) ;*/
	    ...
	    //  System.out.println(stats1.report());
			 ...
			 System.out.println("LES VALEURS A TRACER Mediane avant le calcul");
			 System.out.println("les valeurs a tracer");
			double medianeMC= calculateMedian( moyenneMc);
			
			for (int i =0; i< moyennesRQMCGenerale.length;i++) {
				double medianeRQMC= calculateMedian(moyennesRQMCGenerale[i]);
				double cste=Math.abs(medianeMC-medianeRQMC)/Math.abs(medianeMC);
       		 System.out.println(cste);

			}
			 System.out.println("LES VALEURS A TRACER Mediane apres le calcul de tout");
			 System.out.println("les valeurs a tracer");
  		 ...
  		 double [] aTracer= new double[k];
    		 ...
    		 for (int l=0;l<nbreSet;l++) {
    			 ...
    			 Resultats[l]=(Math.abs(moyenneMc[l]-moyennesRQMCGenerale[m][l]))/(Math.abs(moyenneMc[l]));
    		...
    		/* 
    		 ...
    		 int []vect=matrixToVector(Generamatrix1);
			 
			 DigitalNetBase2 Sob = new SobolSequence(Generamatrix1[0] .length,w,dim,vect);
    		 ...
    		 System.out.println (RQMCExperiment.makeComparisonExperimentMCvsRQMC 
    					( new MCRQMCTEST (dim,w, k,  nbreSet, aTotal[0], uTotal[0]), new MRG32k3a(), Sob, rand1, 1<<Generamatrix1[0] .length, 20));*/
	}
